...
TEST(GraphTest, UnweightedGraphTest){
  ...
  // Creating a Unweighted Directed Graph
...
TEST(GraphTest, PositveWeightedGraphTest){
  ...
  // Creating a Positive Weighted Directed Graph
...
TEST(GraphTest, NegativeWeightedGraphTest){
  ...
  // Creating a negative cycle graph
  ...
  // Creating a Negative Weighted Directed Graph
...
TEST(GraphTest, AcyclicGraphTest) {
  // This test will test the topological sort algorithm for an acyclic graph
  Graph graph;

  // Creating an Acyclic Weighted Directed Graph
  graph.AddEdge("V2", "V0", 4.0);
  graph.AddEdge("V0", "V1", 2.0);
  graph.AddEdge("V1", "V4", 10.0);
  graph.AddEdge("V4", "V6", 6.0);
  ...
  Vertex* vertex = graph.GetVertex("V0");
  ASSERT_EQ(vertex->distance_, 0);
  ...
  vertex = graph.GetVertex("V2");
  ASSERT_EQ(vertex->distance_, std::numeric_limits<double>::max());
